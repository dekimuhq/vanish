...
**Follow-ups (non-blocking):** (1) `verified` proves file integrity only — recon signs with an ephemeral per-report key embedded in the file; key pinning / fingerprint display = recon v2 item. Keep copy honest (never claim tamper-proof — public-claims red line). (2) 9 non-EN native translations of the new `settings.scan*`/`plan.scan*`/`pills.exposure.*` keys (English fallback active — folds into the standing pre-launch native-review item). (3) `SOURCE_TO_CATALOG` map is intentionally thin (2 entries); grows as recon adds probes/brokers.
